...
export const SALARY_CAP = 275000;
...
const ROOKIE_CAP_HITS: number[] = [
  ...
  // Round 3 (picks 65-100 — 36 compensatory picks possible)
  ...
  1160, 1145, 1130, 1115,
  // Rounds 4-7 (picks 101-262): all roughly league minimum
];

/**
 * Get the estimated first-year cap hit for a rookie drafted at the given overall pick.
 * Rounds 4-7 are approximated at the veteran minimum (~$1M).
 */
export function getRookieCapHit(overall: number): number {
  ...
  return 1000; // rounds 4-7 minimum salary
}

/**
 * Per-team salary data. Keyed by team abbreviation, then by lowercase player name.
